(ns paintcrew.actor
  "PaintCrewActor — the ISCO-08 7131 painting-crew job-site
  ...
  housebuilder.actor (and closely on cloud-itonami-isco-9311's
  mininglabor.actor for the physical-safety-domain shape).
  ...
  The unconditional invariant: the Paint Crew Advisor can never
  directly commit an operating record the PaintCrewGovernor refuses,
  and can never make this actor finalize a painting-execution decision
  ...
  regardless of disposition path. This actor coordinates JOB-SITE
  SCHEDULING/LOGISTICS ONLY — it never performs painting work or
  authorizes painting-execution decisions itself."
...
(defn build-graph
  "Build a compiled PaintCrewActor graph. `store` implements
  `paintcrew.store/Store`. `advisor` implements
  `paintcrew.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:painter-id (:painter-id request)
                                    :op (:op proposal)
